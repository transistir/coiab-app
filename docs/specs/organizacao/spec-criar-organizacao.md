...
Projetos que existiam antes da operação não são incorporados, renomeados ou apagados automaticamente. A regra estrutural é: cada organização é composta de dois projetos associados; numa instalação limpa, o total também será dois. Migração de dados legados pertence ao spec A, e não pode ser inferida pela coincidência dos nomes Monitoramento/Alertas.
...
| Limite do nome | **Emenda 2026-09-22 (Q6, plano 2026-09-21):** o limite efetivo é o do **marcador codificado** (`encodeURIComponent`, `CreateOrganization.tsx:92-110`), não o nome cru de 60 unidades — a guarda rejeita nomes cujo marcador mintado exceda o limite, e a mensagem `tooLong` já reflete o limite do marcador. Campo de uma linha; rejeitar quebras de linha/caracteres de controle colados. Não truncar silenciosamente. |
...
| Organização ou operação já existente | Guarda no entry point e no comando, além da UI. Pronta leva à existente; pendente leva à recuperação. Segunda organização é permitida no MVP (#25), mas sua criação/entrada é entrega de UI própria, ainda não implementada. Emenda 2026-09-22 (Q7): a copy de operação em andamento foi resolvida por REUSO de `m.creating` (`ORG.create-creating`, etapa introducao) — copy nova RESOLVIDA/reduzida, sem id canônico novo; mostrar tanto na etapa introducao quanto na navegação de retorno durante operação viva. |
...
1. **D1 — Organização local:** múltiplas organizações por dispositivo no MVP (decisão #25, 09/09/2026), cada uma composta por dois projetos CoMapeo; nenhum novo backend. A interface de criação/entrada em uma segunda organização é entrega própria, ainda não implementada.
