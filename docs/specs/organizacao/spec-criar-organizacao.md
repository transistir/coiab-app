...
| Organização ou operação já existente | Guarda no entry point e no comando, além da UI. Pronta leva à existente; pendente leva à recuperação. Segunda organização é permitida no MVP (#25), mas sua criação/entrada é entrega de UI própria, ainda não implementada. |
