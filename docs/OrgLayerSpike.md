...
branch (see _App-layer implementation_ at the end). The final marker format
...
interruption only; see _What the spike does not cover_).
...
| E2  | Switch between the two projects    | ✅ Pass (core half)                        | Both slots usable through the plain per-project API in either order. Nothing in core changes to "switch" — `activeProjectId` is app state and is not exercised here (see _Not covered_).                                                                                                                                                                                                                                                                                                                                  |
...
| E9  | Marker survives ordinary use       | ⚠️ Hazard proven                           | A plain `EditProjectDetails`-style `$setProjectSettings` save (user text replacing `projectDescription`) erases the marker: reconstruction degrades `ready` → `incomplete`, sibling slot untouched. See _Findings beyond the SPEC_ #4 — the product layer must give the marker a read-only home.                                                                                                                                                                                                                          |
...
single-org spike shortcut. SPEC section 10 requires a _collection_ ("o
...
| E7 sender half                              | Still not simulated end-to-end on devices. The mechanics exist and are unit-tested (retry-only-failed, `ALREADY` = success, receiver-side duplicate collapse per slot), but no integration run drives a sender whose second invite fails mid-fan-out. The **receiver side** of the mid-accept failure is now reachable in the UI: a partial bundle joined against a locally-held slot (join-side recovery) offers Join instead of dead-ending, and provisioning no longer fabricates a slot an invite already covers. |
...
- **E2 UI half and E7 sender-half** as noted above. The join-side recovery
  path, however, is closed at the UI level: a locally-joined slot counts as
  present (`OrganizationInviteReceived` renders Join for a partial bundle
  when the local org holds the other slot), and provisioning never
  fabricates a slot while a pending invite covers the gap
  (`OrganizationProvisioning`). Receiver-side mid-accept recovery is now
  reachable: partial bundle + local slot → Join.
